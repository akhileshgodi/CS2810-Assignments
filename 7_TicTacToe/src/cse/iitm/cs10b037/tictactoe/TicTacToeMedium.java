...
//If it can win it will win. It does not block.
public class TicTacToeMedium implements BotInterface
{
	private int row, col;
	int botInput = 2;
	TicTacToeBoard copyOfBoard;
	int N;
 
	// Makes a new AI with access to Game board
	public TicTacToeMedium(TicTacToeBoard newboard)
	{
    ...
    public int attachWeights(int rowPosition, int colPosition)
    {
        ...
        if (isCenter(rowPosition, colPosition))
            weightage += 10;		//If it can put it at the center. Do that first. Trap can be created.
        if (isCorner(rowPosition, colPosition))
            weightage += 0;		//Least weight to the corner
        Random randGen = new Random();
        return weightage + randGen.nextInt(5); 
    }
